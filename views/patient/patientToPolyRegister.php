...
include '../../models_controllers/patientToPolyRegisterController.php';
...
$sweetAlert2PatientToPolyRegister = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['register_to_poly'])) {
        $schedule_id = $_POST['schedule_id'];
        $complaint = $_POST['complaint'];

        $result = registerPatientToPoly($conn, $patientId, $schedule_id, $complaint);

        if ($result) {
             $sweetAlert2PatientToPolyRegister = 
                "Swal.fire({
                    title: 'Success!',
                    text: 'Register to poly successfull. Your queue number has been generated.',
                    icon: 'success'
                }).then(() => {
                    window.location.href = 'patientToPolyRegister.php';
                });";
        } else {
            $sweetAlert2PatientToPolyRegister = 
                "Swal.fire({
                    title: 'Error!',
                    text: 'Register to poly failed. Please try again.',
                    icon: 'error'
                }).then(() => {
                    window.location.href = 'patientToPolyRegister.php';
                });";
        ...
        <div class="row w-100 position-absolute" style="margin: 0; max-width: 100%;">
            ...
            <div class="col-lg-10 div-two">
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item d-none d-md-block"><a href="#" class="nav-link">Register To Poly</a></li>
                </ul>
            ...
            <!--begin::App Main-->
                    <div class="container-fluid pe-5 w-100">
                        <div class="row justify-content-center">
                            <div class="col-lg-12">
                                <h2 class="mt-4">Register To Poly</h2>
                                    <form method="POST" action="patientToPolyRegister.php">
                                        <label class="mt-3" for="schedule_id">Select Poly Schedule :</label>
                                        <select id="schedule_id" name="schedule_id" class="px-2 py-2 w-100" required>
                                            <option value="" selected disabled>Select Poly Schedule</option>
                                        <?php
                                            $query = "SELECT c.id AS schedule_id, c.day, c.start_time, c.end_time, p.poly_name, d.name AS doctor_name
                                                    FROM check_schedule c
                                                    JOIN doctor d ON c.doctor_id = d.id
                                                    JOIN poly p ON d.poly_id = p.id";

                                            $result = $conn->query($query);
                                            while ($schedule = $result->fetch_assoc()) {
                                                $schedule_details = $schedule['poly_name'] . ' - ' . $schedule['doctor_name'] . ' (' . $schedule['day'] . ': ' . $schedule['start_time'] . ' - ' . $schedule['end_time'] . ')';
                                                echo "<option value='{$schedule['schedule_id']}'>{$schedule_details}</option>";
                                            }
                                        ?>
                                        </select>
                                        <label class="mt-4" for="complaint">Complaint :</label>
